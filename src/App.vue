...
const metadata = {
  name: 'Sushi Cards',
  description: 'Sushi cards matching game',
  url: 'http://localhost:3000', // url must match your domain & subdomain
...
function handleLose() {
  ...
  setTimeout(() => {
    alert('The slot is full, keep up the good work~')
...
<template>
  <div flex flex-col w-full h-full>
    ...
    <div ref="containerRef" flex-1 flex>
      ...
      <transition name="bounce">
        <div v-if="isWin" color="#000" flex items-center justify-center w-full text-28px fw-bold>
          Successfully Joined Sushi Cards~
        </div>
      </transition>
      <transition name="bounce">
    ...
    <div flex items-center mx-auto justify-center gap-4 p-4>
      <button :disabled="removeFlag"  @click="handleRemove">
        Move out first three
      </button>
